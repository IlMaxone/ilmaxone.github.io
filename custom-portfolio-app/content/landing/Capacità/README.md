...
4. indicare in `route` un percorso interno che inizi con `/`;
5. eseguire `npm run generate:experiences` oppure direttamente `npm start`, `npm test` o `npm run build`.

Lo script valida i file, li ordina tramite `order` e genera il catalogo usato dalla pagina Capacità. Le coordinate X/Y/Z vengono calcolate automaticamente, quindi non occorre modificare il componente Angular.
